import { getBundledCaptain } from "@/lib/captainPhotoBundle";
...
export async function getCaptainForTeam(teamName, teams = null) {
  ...
  const bundled = getBundledCaptain(canonical);
  if (bundled?.photo) {
    const value = {
      name: displayCaptainName(bundled.name),
      photo: bundled.photo,
    };
  ...
  const captainPlayer =
    findCaptainFromSquad(data.squad, canonical) ??
    (bundled?.name ? { name: bundled.name } : null);
  ...
  const resolvedPhoto = await resolvePlayerPhoto(
    captainPlayer.name,
    canonical,
  );
  const value = {
