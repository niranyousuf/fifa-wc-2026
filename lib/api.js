...
async function wcFetch(path, { revalidate = CACHE_REVALIDATE_SECONDS } = {}) {
...
export const getTeamsList = cache(async function getTeamsList() {
  const teams = await wcFetch(`/teams?tournament=${WC_YEAR}`);
...
export function normalizeMatch(match, flags = {}, teams = [], rawMatchByNumber = new Map()) {
  const hasScores =
    match.homeScore !== null &&
    match.homeScore !== undefined &&
    match.awayScore !== null &&
    match.awayScore !== undefined;
  const finished = hasScores || match.status === "finished";

  const homeRaw =
    match.homeTeam ?? match.home ?? match.homeRef ?? match.home ?? "TBD";
  const awayRaw =
    match.awayTeam ?? match.away ?? match.awayRef ?? match.away ?? "TBD";

  const homeName = resolveCanonicalTeamName(
    normalizeBracketRef(homeRaw, match.matchNo, rawMatchByNumber, teams),
    teams,
  );
  const awayName = resolveCanonicalTeamName(
    normalizeBracketRef(awayRaw, match.matchNo, rawMatchByNumber, teams),
    teams,
  );
...
function normalizeBracketRef(rawName, matchNo, rawMatchByNumber, teams) {
  if (!rawName || typeof rawName !== "string") {
    return rawName;
  }

  const feederMatch = rawName.trim().match(/^([WL])(\d+)$/i);
  if (!feederMatch) {
    return rawName;
  }

  const [, type, feederNo] = feederMatch;
  const feederMatchNo = Number(feederNo);
  if (!Number.isFinite(feederMatchNo)) {
    return rawName;
  }

  const referencedMatch = rawMatchByNumber.get(feederMatchNo);
  if (!referencedMatch) {
    return rawName;
  }

  const isWinnerRef = type.toUpperCase() === "W";
  const finished =
    referencedMatch.homeScore !== null &&
    referencedMatch.homeScore !== undefined &&
    referencedMatch.awayScore !== null &&
    referencedMatch.awayScore !== undefined;

  if (!finished) {
    return rawName;
  }

  const teamName = isWinnerRef
    ? referencedMatch.homeScore > referencedMatch.awayScore
      ? referencedMatch.homeTeam ?? referencedMatch.home ?? referencedMatch.homeRef
      : referencedMatch.awayTeam ?? referencedMatch.away ?? referencedMatch.awayRef
    : referencedMatch.homeScore > referencedMatch.awayScore
      ? referencedMatch.awayTeam ?? referencedMatch.away ?? referencedMatch.awayRef
      : referencedMatch.homeTeam ?? referencedMatch.home ?? referencedMatch.homeRef;

  return teamName || rawName;
}
...
function normalizeFixtures(matchesResponse, flags, teams, { status } = {}) {
  ...
  const rawMatchByNumber = new Map(
    matches
      .map((match) => [Number(match.matchNo), match])
      .filter(([matchNo]) => Number.isFinite(matchNo)),
  );

  let fixtures = matches.map((match) =>
    normalizeMatch(match, flags, teams, rawMatchByNumber),
  );
...
export async function getHubBundle() {
  try {
    const [standingsResponse, matchesResponse, teams] = await Promise.all([
      wcFetch(`/standings?year=${WC_YEAR}`),
      wcFetch(`/matches?year=${WC_YEAR}`),
      getTeamsList(),
    ]);
...
const MATCH_STAT_FIELDS = [
  ...
  ["shotsTotal", "Total Shots", false],
  ["shotsOnGoal", "Shots on Goal", false],
  ["shotsOffGoal", "Shots off Goal", false],
  ["corners", "Corner Kicks", false],
  ["offsides", "Offsides", false],
  ["fouls", "Fouls", false],
  ["yellowCards", "Yellow Cards", false],
  ["redCards", "Red Cards", false],
  ["goalkeeperSaves", "Goalkeeper Saves", false],
  ["passesTotal", "Total Passes", false],
  ["passesAccurate", "Accurate Passes", false],
  ["passesPct", "Pass Accuracy", true],
  ["expectedGoals", "Expected Goals (xG)", false],
];

function goalEventDetail(goal) {
  switch (goal.type) {
    case "penalty":
      return "Penalty";
    case "own_goal":
...
function cardEventDetail(card) {
  if (card.color === "red") return "Red Card";
  if (card.color === "yellow") return "Yellow Card";
  return card.type ?? "Card";
}

function eventTeamName(fixture, side) {
  if (side === "home") return fixture.teams.home.name;
  if (side === "away") return fixture.teams.away.name;
  return side;
}

function goalsToEvents(match, fixture) {
  ...
  const goalEvents = goals.map((goal) => ({
    ...
    team: { name: eventTeamName(fixture, goal.team) },
    detail: goalEventDetail(goal),
  ...
  const cardEvents = cards.map((card) => ({
    ...
    team: { name: eventTeamName(fixture, card.team) },
    detail: cardEventDetail(card),
...
function normalizeMatchStatistics(match, fixture) {
  const stats = match.statistics;
  if (!stats?.home || !stats?.away) return [];

  const buildSide = (sideStats, team) => ({
    team: {
      id: team.id,
      name: team.name,
      logo: team.logo,
    },
    statistics: MATCH_STAT_FIELDS.flatMap(([key, label, isPercent]) => {
      const raw = sideStats[key];
      if (raw === null || raw === undefined) return [];

      return [
        {
          type: label,
          value: isPercent ? `${raw}%` : raw,
        },
      ];
    }),
  });
  ...
  return [
    buildSide(stats.home, fixture.teams.home),
    buildSide(stats.away, fixture.teams.away),
  ];
}

function enrichMatchFromList(direct, fromList) {
  if (!fromList) return direct;
  if (!direct) return fromList;

  return {
    ...fromList,
    ...direct,
    homeScore: direct.homeScore ?? fromList.homeScore,
    awayScore: direct.awayScore ?? fromList.awayScore,
    goals: direct.goals?.length ? direct.goals : fromList.goals,
    cards: direct.cards?.length ? direct.cards : fromList.cards,
    statistics: direct.statistics ?? fromList.statistics,
    status: direct.status ?? fromList.status,
    attendance: direct.attendance ?? fromList.attendance,
    referee: direct.referee ?? fromList.referee,
    weather: direct.weather ?? fromList.weather,
    lineups: direct.lineups ?? fromList.lineups,
  };
}

async function loadYearMatchList() {
  const listResponse =
  ...
  return extractMatchesList(listResponse);
}

function findMatchInList(matches, normalizedId) {
  return (
    matches.find(
...
async function resolveMatchRaw(matchId) {
  ...
  const matches = await loadYearMatchList();
  const fromList = findMatchInList(matches, normalizedId);

  let direct = null;
  try {
    direct = await wcFetch(`/matches/${encodeURIComponent(normalizedId)}`, {
      revalidate: 60,
    });
  ...
  const merged = enrichMatchFromList(direct, fromList);
...
export async function getMatch(id) {
  ...
  return {
    fixture,
    statistics: normalizeMatchStatistics(match, fixture),
    events: goalsToEvents(match, fixture),
    raw: match,
  };
}
